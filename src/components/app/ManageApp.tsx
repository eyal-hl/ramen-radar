...
function TextField({ label, value, onInput, required = false, type = 'text', readOnly = false }: { label: string; value: string | number; onInput: (value: string) => void; required?: boolean; type?: string; readOnly?: boolean }) {
  return <label><span>{label}</span><input type={type} value={value} required={required} readOnly={readOnly} onInput={(event) => onInput(event.currentTarget.value)} /></label>;
}
...
export default function ManageApp() {
  ...
  const choose = (place: FirestorePlace) => { setSelected(place); setIsNew(false); };
  const saved = (place: FirestorePlace) => { setPlaces((current) => [...current.filter(({ id }) => id !== place.id), place].sort((a, b) => a.name.localeCompare(b.name))); setSelected(place); setIsNew(false); };
  ...
  return <main class="manage-shell shell"><header class="manage-top"><div><p class="eyebrow">Private editor</p><h1>Manage Ramen Radar</h1><p>Signed in as {user.email ?? user.displayName ?? user.uid}</p></div>{!fixtureMode && <button onClick={() => void signOutUser()}>Sign out</button>}</header><div class="manage-layout"><aside class="manage-sidebar"><button class="manage-primary" onClick={() => { setSelected(newPlace()); setIsNew(true); }}>Add place</button>{places.length === 0 && <button onClick={() => void importSeed()}>Import existing JSON places</button>}<nav aria-label="Places">{places.map((place) => <button class={selected?.id === place.id ? 'active' : ''} onClick={() => choose(place)} key={place.id}><span>{place.name}</span>{place.archived && <small>Archived</small>}</button>)}</nav></aside><section>{selected ? <PlaceEditor key={`${selected.id}-${selected.updatedAt ?? 'new'}`} initial={selected} isNew={isNew} onSaved={saved} /> : <div class="manage-empty"><h2>Choose a place</h2><p>Select an existing place or add a new one.</p></div>}</section></div></main>;
}
