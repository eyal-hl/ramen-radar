...
export default function DirectoryApp() {
  ...
  const [view, setView] = useState<'list' | 'map'>(() => typeof location !== 'undefined' && new URLSearchParams(location.search).get('view') === 'map' ? 'map' : 'list');
  ...
  return <main>
    <section class="hero shell"><div class="hero__copy"><p class="eyebrow">A personal noodle field guide</p><h1>Good ramen deserves<br /><em>proper notes.</em></h1><p class="hero__intro">Tracking bowls across Givatayim and Tel Aviv—what we want to try, what we ordered, and whether the broth earned a return trip.</p></div><dl class="hero__stats"><div><dt>Places</dt><dd>{String(places.length).padStart(2, '0')}</dd></div><div><dt>Visited</dt><dd>{String(visited).padStart(2, '0')}</dd></div><div><dt>Visits</dt><dd>{String(visits).padStart(2, '0')}</dd></div></dl></section>
    <section class="directory shell" aria-labelledby="directory-title">
      <div class="section-heading section-heading--row"><div><p class="eyebrow">The running list</p><h2 id="directory-title">Ramen places</h2></div><div class="directory-heading-tools"><p aria-live="polite">{visible.length} {visible.length === 1 ? 'place' : 'places'}</p><div class="view-toggle" aria-label="Directory view"><button type="button" aria-pressed={view === 'list'} onClick={() => setView('list')}>List</button><button type="button" aria-pressed={view === 'map'} onClick={() => setView('map')}>Map</button></div></div></div>
      <form class="directory-filters" aria-label="Filter ramen places" onReset={() => setFilters(emptyFilters)}>
        <label class="search-field"><span>Search the radar</span><input type="search" value={filters.query} onInput={(event) => update('query', event.currentTarget.value)} placeholder="Place, city, style, tag…" /></label>
      ...
      {view === 'list' ? <div class="place-grid">{visible.map((place) => <PlaceCard place={place} key={place.id} />)}</div> : <DirectoryMap places={mapPlaces} visibleIds={visible.map(({ id }) => id)} />}
      {visible.length === 0 && <div class="empty-state"><span aria-hidden="true">∅</span><h3>No bowls on this frequency</h3><p>Try clearing a filter or searching for something broader.</p></div>}
    </section>
  </main>;
}
